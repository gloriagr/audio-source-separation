...
def reconstruct(phase, bass_mag, vocals_mag, drums_mag, others_mag, song_num, segment_num, destination_path):
    ...
    # Save as wav files
    librosa.output.write_wav(os.path.join(destination_path, 'vocals', str(song_num) + '_' + str(segment_num) + '.wav'),
                             vocals_audio, sr=44100)
    librosa.output.write_wav(os.path.join(destination_path, 'bass', str(song_num) + '_' + str(segment_num) + '.wav'),
                             bass_audio, sr=44100)
    librosa.output.write_wav(os.path.join(destination_path, 'drums', str(song_num) + '_' + str(segment_num) + '.wav'),
                             drums_audio, sr=44100)
    librosa.output.write_wav(os.path.join(destination_path, 'others', str(song_num) + '_' + str(segment_num) + '.wav'),
                             others_audio, sr=44100)
    return
